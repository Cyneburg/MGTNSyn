...
class TestbedDataset(InMemoryDataset):
    ...
    def process(self, xd, xt, xt_featrue, y, smile_graph):
        ...
        for i in trange(data_len):
            ...
            laplacian_positional_encoding = torch.from_numpy(np.real(EigVec[0:c_size,0:8])).float()
            # c_size用于删除最后一个添加的原子

            # if laplacian_positional_encoding.shape[0] < min_la:
            #     min_la = laplacian_positional_encoding.shape[0]
            # if laplacian_positional_encoding.shape[0] > max_la:
            #     max_la = laplacian_positional_encoding.shape[0]
            ...
            GCNData = Data(x=torch.Tensor(features),
                           ...
                           #y=torch.Tensor([labels]),#Classification
                           y=torch.FloatTensor([labels]),#Regression
            ...
            smile_str = smile_graph[smiles][:128]
            features = self.drug_model.encode(smile_str)
            mol = Chem.MolFromSmiles(smile_graph[smiles])
            ...
            for n in cell:
                new_cell.append(float(n))
            GCNData = Data(compounds=torch.FloatTensor([features]),
